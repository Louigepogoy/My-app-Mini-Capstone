...
export default function OwnerBookingsPage() {
  ...
  if (loading) return <p className="text-slate-600">Loading booking requests…</p>;

  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-slate-900">View Booking Requests</h1>
      <p className="text-slate-600">Approve or reject customer booking requests.</p>
      <div className="space-y-2">
        {list.length === 0 ? (
          <p className="text-slate-600">No booking requests.</p>
        ) : (
          list.map((b) => (
